...
// - フォーカス中のセルで → を押すと右隣の日セルへフォーカスが移り、
//   Tab ストップ（tabIndex=0）もそのセルへ追従する
```
...
| キー | 動作 |
| --- | --- |
| `←` / `→` | 読み順で前後のセルへ移動（行末では次/前の行へ、複数月・年ビューでは隣の月グリッドへ連続移動） |
...
| `Enter` | セル内にその日を開始日とする予定があれば最初の予定へフォーカスを移す。なければその日 1 日分の範囲選択（作成） |
...
- **Enter でセルから予定へ入る** — セルの Enter はセル内の最初の予定へフォーカスを移します。複数日にまたがる帯は開始日のセルが所有するため、Enter で入れるのは「その日を開始日とする予定」です（開始日以外の日からは Tab で到達できます）
- **Escape で予定からセルへ戻る** — 予定にフォーカスがある状態の Escape は、その予定の開始日のセルへフォーカスを戻します（ドラッグ操作中の Escape はドラッグのキャンセルとして動作します）
