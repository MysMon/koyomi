...
は発生の開始時刻を計算する低レベル API で、`getOccurrences` や各種ビュー
はこれを内部で利用しています）。
...
// UNTIL（終了日時指定。イベント TZ の壁時計として解釈される。後述）
expandRecurrence({
...
// => 7/1, 7/2, 7/3, 7/4, 7/5（UNTIL の発生自身を含む、5 回）
```
...
- `occurrenceStart` — 対象発生の（現在の）開始時刻
- `scope` — `'this'`（この予定のみ）/ `'thisAndFollowing'`（これ以降のすべて）/ `'all'`（すべて）

### scope: 'this' — この予定のみ

対象発生を、`recurringEventId`（マスターの ID）と `originalStart`（本来の
開始日時）を持つ**単発のオーバーライドイベント**として切り出します。マスター
自身は変更されません。既にオーバーライド済みの発生への再変更は、新しい
...
// 7/3 の発生だけ 10:00 に変更する（occurrenceStart は 7/3 09:00 JST の絶対時刻）
...
// override.originalStart === 7/3 09:00 JST（置き換えた元の発生時刻）
// override.start === '2026-07-03T10:00:00'（patch した値がそのまま入る）

// 既にオーバーライド済みの発生を直接編集する場合は、単発イベントと同様に
...
元の繰り返しを対象発生の直前で打ち切り（`UNTIL` を設定）、対象発生以降を
**新しい繰り返しイベント（新しい ID）として分割**します。`COUNT` が
指定されている場合は消化済み回数を差し引いた値を新シリーズに引き継ぎます。
対象発生以降（`>=`）の EXDATE・オーバーライドは新シリーズに付け替えられます。

**対象が最初の発生の場合は `'all'` と同じ扱い**になり、シリーズ分割は
起きません。
...
最初の発生を対象にした場合は分割が起きません。

```ts
// 最初の発生（7/1）を対象に thisAndFollowing を指定すると 'all' と同じ扱いになる
calendar.updateEvent(
  'standup',
  { title: '朝会（改称）' },
...
- `target` 省略（または `scope: 'all'`）— 繰り返し全体と、それを参照する
  すべてのオーバーライドを取り除く
- `scope: 'this'` — 対象発生を **EXDATE に追加**する。対象が既にオーバー
  ライド済みの発生の場合は、そのオーバーライドを取り除いて元発生の EXDATE
  に追加する
- `scope: 'thisAndFollowing'` — 対象発生の直前で繰り返しを打ち切り、対象
  発生以降のオーバーライド・EXDATE を取り除く。対象が最初の発生なら
  繰り返し全体を削除する

`id` にオーバーライドの ID を渡した場合、`scope: 'this'`（または `target`
省略）は**そのオーバーライドを取り除き、元発生（`originalStart`）を親の
...
ドラッグ＆ドロップによる発生の移動・リサイズも内部的には `updateEvent` を
...
- **`exdates`** — マスターイベントが持つ、繰り返しから除外する発生の開始
  日時の配列（EXDATE 相当）。「この予定のみ削除」した発生がここに追加
...
- **`originalStart`** — オーバーライドイベントが持つ、置き換え対象となる
  発生の本来の開始日時。イベント展開時にはマスターの発生のうち
...
参照先のマスターイベントが見つからない「孤児オーバーライド」（`setEvents`
の一部だけを渡した場合など）は、防御的に単発イベントとして扱われます。

オーバーライドの `start` / `originalStart` を**オフセットなしの文字列**で渡す
場合の解釈タイムゾーンは「オーバーライド自身の `timeZone` → マスターの
`timeZone` → 表示タイムゾーン」の順にフォールバックします。マスターに明示的な
`timeZone` を設定している場合、オーバーライド側で `timeZone` を省略しても
マスターと同じ壁時計解釈になります（展開時・編集時とも同じ規則）。

## RDATE — パターン外の発生を追加する（rdates）

`rrule` のパターンに乗らない日時へ発生を**追加**したい場合は、`rdates`
（RFC 5545 の RDATE 相当）を使います。`exdates` と対称のフィールドです。

- `rrule` と併用でき、`rrule` なしで `rdates` のみの指定も可能
  （その場合は `start` の発生 ＋ 各 `rdates` の発生に展開されます）
- `exdates` と同じ日時が指定された場合は**除外が優先**されます
- 発生の長さはマスターの発生と同じ（`end - start`、なければ既定長）
...
// 期待される動作:
// - 水曜（7/1、7/8、…）の発生に加えて 7/3（金）9:00 の発生が展開される
// - rdate 由来の発生も isRecurring: true で、「この予定のみ変更/削除」の対象になる
```

## タイムゾーンとの関係

繰り返しは「イベントのタイムゾーンにおける壁時計時刻」を維持して展開
...
**注意:** RRULE の `UNTIL` はイベントのタイムゾーンにおける**壁時計時刻**
...
// 「イベント TZ（America/New_York）の壁時計 9:00」として扱われる。
...
`UNTIL` を指定する際は、打ち切りたい発生の**イベント TZ での壁時計時刻**を
