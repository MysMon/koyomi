# テスト規約（内部開発者向け）

Koyomi のテストの位置づけ・書き方・置き場所を定める。用語は [terminology.md](./terminology.md) に従う。

## テストの位置づけ

- **テストが仕様である。**「こうあるべき」という振る舞いの定義はテストで行い、テストが固定する。仕様を変えるときはテストを先に変える（TDD の Red はその宣言である）
- **docs/*.md は利用者向けの公開情報であり、テストとは別の成果物。**利用者に影響する振る舞いを確定・変更したときは docs にも反映し、テストが担保する挙動と矛盾させない（矛盾を見つけたら docs 側を直す）
- **テストから docs への参照・リンクは書かない。**テストの期待値はテスト自体で完結して読めること。自明でない期待値（DST 切替日の具体的な UTC 時刻、ISO 週番号の年跨ぎなど）には、値の**計算根拠**を短いコメントで添えてよい

## `.test.ts` と `.spec.ts` の使い分け

- **`.test.ts(x)`** — Vitest で実行する単体・結合テスト（jsdom 含む）。本リポジトリのテストは現在すべてこれに該当する
- **`.spec.ts(x)`** — `e2e/` 配下で Playwright が実行する実ブラウザ E2E 専用。Vitest のテストに `.spec` を使わない
- つまり接尾辞は「テストの実行レイヤー」を表す。同じレイヤー内での目的の違い（振る舞いの確定か回帰防止か等）は接尾辞・ファイル分割で表現しない

## 置き場所と構造

- Vitest のテストは**実装ファイルと同階層の `*.test.ts(x)`**（1 実装ファイルにつき 1 テストファイル）。目的別の別ファイルや専用ディレクトリは作らない
- `pnpm test:pairs` が実装と同名の `.test.ts` / `.test.tsx` の存在を検査する。型宣言だけの `types.ts` と公開 barrel の `index.ts` は対象外
- **describe はテスト対象の振る舞い・機能で分類する。**経緯・手法（「リグレッション」「監査で追加」等）によるグループ化はしない
- 複数モジュールにまたがる検証は、対象モジュールごとに分割してそれぞれのテストファイルへ置く

## 書き方

- **TDD 必須**（CLAUDE.md）: 実装より先に失敗するテストを書き、Red を確認してから実装する（Red → Green → Refactor）
- テスト名は日本語で「入力・条件 → 期待される振る舞い」が読み取れる文にする
- 実装詳細の意図的なピン留め（キャッシュの参照同一性・memo の再レンダー抑制・共有参照によるアロケーション回避など）は、それが性能・整合性のための固定であることがテスト名かコメントから分かるようにする

## 実行環境の前提

- テストプロセスは `TZ=Asia/Tokyo` 固定（`vitest.config.ts`）。他タイムゾーンの検証は `TZDate` や明示的な IANA ID 指定で行う
- DOM を伴うテストは jsdom。`getBoundingClientRect` / `elementsFromPoint` / `PointerEvent` など jsdom が実装しない API は既存テストのモックパターン（`mockRect` / `mockElementsFromPoint` / `MouseEvent` 代用）に倣う
- 単一ファイルの実行: `pnpm --filter @koyomi-cal/react exec vitest run <path>`。コミット前は必ず `pnpm check`
- 実ブラウザ E2E: `pnpm test:e2e`。Chromium / Firefox / WebKit で主要操作、Chromium の実タッチ入力、仮想化スクロールとフォーカス保持、キーボードのみでの予定操作（矢印キー移動・リサイズ・Delete 削除・aria-live 通知）、`gridNavigation`（roving tabindex）のセル間移動とモード分離、繰り返し編集スコープ × DST 切替日の組み合わせ、axe による WCAG 2.0/2.1/2.2 A/AA の自動検査を行う

## 性能ベンチ（`pnpm bench` / `pnpm bench:ci`）

- `pnpm bench` は `bench/` 配下の Playwright ベースの計測スイートを実行し、代表構成ごとの初回描画時間・スクロール性能を `bench/results/` に記録する（詳細・実測値は `docs/performance.md` の「性能ベンチマーク」節）
- `pnpm bench:ci` は最重量構成（イベント 10,000 件 × リソース 1,000 件）のみを `bench/regression.spec.ts` の粗い閾値（`RENDER_LIMIT_MS` / `SCROLL_AVG_FRAME_LIMIT_MS`）と比較するリグレッション検出で、CI で必須（コミット前は任意）
- **`pnpm bench:ci` が失敗したときの切り分け手順**:
  1. まず再実行する。CI ランナーの負荷変動による flaky な超過は再実行で解消することが多い
  2. 再実行しても失敗する場合、実装側の計算量退行（仮想化の無効化、O(n²) 化など）を疑って調査する。原因があれば実装を直す
  3. 意図した設計変更（新機能の追加など）による正当な性能低下だと判断できる場合のみ、次の手順でしきい値を更新する
- **しきい値の更新手順**: `pnpm bench` を再実行して新しい中央値を取り、その約 10 倍（スクロールは約 30 倍）を新しい閾値とする。`bench/regression.spec.ts` の `RENDER_LIMIT_MS` / `SCROLL_AVG_FRAME_LIMIT_MS` と、`docs/performance.md` の「性能ベンチマーク」節の実測値表・計測日を**同じ PR でセットで**更新する（どちらか一方だけの更新は行わない）

## バンドルサイズ検証（`pnpm bundle:check`）

- `pnpm build && pnpm bundle:check`（`scripts/check-bundle-size.mjs`）は、ビルド済み dist を最小アプリでバンドルし、tree-shaking の実効性とライブラリ寄与分の gzip サイズ（`GZIP_BUDGET_KB`）を検証する
- **失敗時の切り分け**: `FORBIDDEN_IN_MONTH_ONLY` のマーカー文字列が月ビューのみのバンドルに含まれる（tree-shaking が壊れている）失敗と、`GZIP_BUDGET_KB` の超過（サイズ増加）失敗のどちらかをログの内容で判別する。前者はバンドル構成（`sideEffects`・import 経路）の変更が原因なので実装を直す
- **`GZIP_BUDGET_KB` の更新基準**: 後者が機能追加による正当な増加だと判断できる場合のみ、`pnpm build && pnpm bundle:check` の実測 gzip サイズ + 約 1 割を新しい `GZIP_BUDGET_KB` とする。`scripts/check-bundle-size.mjs` の定数と、`docs/performance.md` の「バンドルサイズと tree-shaking」節の実測値表・計測日を**同じ PR でセットで**更新する

## カバレッジの考え方

- 境界条件を優先する: 範囲の両端（ちょうど / 1 つ超過）、排他端（`end`・`endTime`）、DST の切替日（存在しない時刻・二重に存在する時刻）、年末年始・うるう年、空配列・0 件・`undefined` と空配列の意味差、`editable: false`
- **DST は「春の進み」と「秋の巻き戻り」の両側を検証する**。巻き戻り日（実時間 25 時間、同じ現地時刻が 2 回現れる: `America/New_York` 2026-11-01 等）と、**深夜 0:00 に切り替わり「日の開始」が 0:00 でなくなる日**（`America/Santiago` 2026-09-06、`Africa/Cairo`、`America/Havana` 等）は、進む側のテストだけでは検出できない欠陥を持ちやすい
- **タイムゾーン依存の機能は「実行環境のローカル TZ（`Asia/Tokyo`）と対象 TZ が異なる」構成で検証する**。ローカル TZ と同じ TZ だけのテストは、実装が暗黙にローカル TZ へ依存していても通ってしまう。特に、ローカルより**東**のゾーン（`Australia/Sydney`、`Pacific/Auckland` 等）は西のゾーンとオフセット差の符号が逆になり、片側だけでは対称な欠陥を見逃す
- 省略可能なオプション・コールバックは「省略時に既定の挙動・DOM 出力が変わらないこと」を必ず対で検証する
- コールバックは「発火する条件」と「発火しない条件」（キャンセル・拒否・対象外ビュー）を対で検証する
