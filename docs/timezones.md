...
終日イベントの `start` / `end` は `'YYYY-MM-DD'` の日付キー文字列で持つのが基本です。
`Date` などの絶対時刻を渡した場合、その絶対時刻が属する「日付」はイベントの
`timeZone`（なければ表示タイムゾーン）における日付として解釈されます。ビルトインの
ドラッグ・キーボード操作は終日イベントの変更を常に日付キー文字列で書き込むため、
この解釈の影響を受けません。
...
### DST 切替日のビュー表示（週・日・リソース・タイムライン）

時間グリッド（週・日・リソース）とタイムラインの各日は、実時間が 23 時間や
25 時間になる DST 切替日でも常に 24 時間（1440 分）のグリッドとして表示されます。
予定は開始・終了それぞれの現地時刻の位置に配置されます。

秋の巻き戻り日には同じ現地時刻が 2 回現れるため、2 回現れる時間帯の 2 回目に
終わる予定は、終了の現地時刻の位置が開始を下回ります（例: `America/New_York` の
2026-11-01 における 01:30 EDT 開始 → 01:00 EST 終了の 30 分の予定）。この場合は
開始の現地時刻の位置から**実時間の長さ**を保った区間として表示されます（30 分の予定は
30 分の高さ・幅になる）。2 回現れる時間帯をまたいで終わる予定は、開始・終了とも
現地時刻の位置に配置されます（区間は実時間より短く見えます）。
