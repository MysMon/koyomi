...
describe('layoutTimeGridItems', () => {
  ...
  describe('minSlotMinutes（重なり判定上の最小長さ）', () => {
    it('既定 30 分の実効長により 15 分イベント 2 件（10:00-10:15 と 10:20-10:35）が横並びになる', () => {
      // 実時間では重ならないが、実効長 30 分（10:00-10:30 と 10:20-10:50）では重なる。
      const result = layoutTimeGridItems([
        item('a', t(10), t(10, 15)),
    ...
    it('実効長どうしの比較も [start, end) 排他（15 分イベントが 30 分ちょうど離れていれば重ならない）', () => {
      // a の実効区間は 10:00-10:30。b は 10:30 開始なので排他比較で重ならない。
      const result = layoutTimeGridItems([
        item('a', t(10), t(10, 15)),
    ...
    it('minSlotMinutes を明示指定した場合はその値が使われる', () => {
      // 実効長 60 分: a は 10:00-11:00 とみなされ b (10:45-11:45) と重なる。
      const result = layoutTimeGridItems(
    ...
    it('minSlotMinutes より長いイベントの重なり判定は実際の長さで行われる', () => {
      // 60 分イベントどうしが連続しても重ならない（実効長の影響を受けない）。
      const result = layoutTimeGridItems([item('a', t(10), t(11)), item('b', t(11), t(12))], {
        minSlotMinutes: 30,
      });
  ...
  describe('不正入力（startMinutes < endMinutes の前提違反）', () => {
    it('長さ 0 のアイテムは例外を投げず、実効長 30 分として扱われる', () => {
